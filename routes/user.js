...
router.post('/login', (req, res) => {
    ...
    if (username === 'admin' && password === 'admin') {
        db.get("SELECT * FROM users WHERE username = 'admin'", (err, adminRow) => {
            ...
            req.session.save((err) => {
                ...
                return res.send("<script>alert('👑 최고 관리자 계정으로 로그인이 완료되었습니다. 👑'); location.href='/stud7/';</script>");
            });
        });
        return;
    }
    ...
    db.get("SELECT * FROM users WHERE username = ? AND password = ?", [username, password], (err, row) => {
        ...
        req.session.save((err) => {
            if (err) console.error(err);
            res.send("<script>alert('🔓 로그인이 성공적으로 완료되었습니다.'); location.href='/stud7/';</script>");
        });
    });
});
...
router.get('/logout', (req, res) => {
    req.session.destroy((err) => {
        ...
        res.send("<script>alert('🚪 안전하게 로그아웃 되었습니다.'); location.href='/stud7/';</script>");
    });
});
...
router.post('/register', (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE username = ?", [username], (err, row) => {
        ...
        db.run(query, [username, password, name, phone, address], function (err2) {
            ...
            res.send("<script>alert('🎉 회원가입이 성공적으로 완료되었습니다! 로그인해 주세요.'); location.href='/stud7/user/login';</script>");
        });
    });
});
